...
Originated and led by [Vlad Ryzhkov](AUTHORS.md). Public since 2026-07-14; execution has begun. The program's headline result is now methodological, not behavioral: a post-verdict adversarial audit (Mission X) found that the measuring instrument — not the model — was the modal failure class of the whole revision line. E5's registered refutation of mechanical reclosure is **WITHDRAWN and REOPENED**: the Arm-B-vs-Arm-C contamination separation it rested on does not exist under a valid instrument (it is an NLI template-collision artifact, confirmed five independent times in this program alone), and the correction is drafted and awaiting publication ([correction note](experiments/E8-instruction-breakpoint/coldstart-package/x1-anatomy/CORRECTION-NOTE-DRAFT.md), [standalone finding](experiments/E8-instruction-breakpoint/coldstart-package/mission-x/FINDING-NLI-ARTIFACT.md)). The prior registered snapshot remains archived under DOI [10.5281/zenodo.21399411](https://doi.org/10.5281/zenodo.21399411) but is superseded pending a new version. Under the corrected instrument, instructed revision has no measured failure regime anywhere tested: single-shot (E5), depth (E8's A1V2 rebuild, true floor 0/447), and repeated near-lossless summarization (E9, 0/1800 real — though E9's registered compression band was not achieved, leaving real-ratio compaction open). E3 (future-output volume) remains `confirmed-shaped` at exploratory grade. The remaining experiments are pre-execution, and that is still the point: the plan is public so it can be attacked before execution hardens its assumptions ([STATUS.md](STATUS.md), [LOG.md](LOG.md)).
...
| [E5](experiments/E5-reclosure/) | Mechanical context rebuild beats instructed disregard | REOPENED — the 2026-07-16 registered REFUTED verdict is withdrawn: the B-vs-C separation is an NLI instrument artifact, not a real effect; [correction note](experiments/E8-instruction-breakpoint/coldstart-package/x1-anatomy/CORRECTION-NOTE-DRAFT.md), [results record](results/E5-reclosure/2026-07-15-registered-run/) |
...
| [E8](experiments/E8-instruction-breakpoint/) | Instructed revision degrades along measurable difficulty axes (no operators) | Ran to verdict 2026-07-19: Block B, no axis broke; depth axis (A1) was invalid as registered (polarity inversion) and rebuilt — [A1V2-DEPTH-REBUILD.md](experiments/E8-instruction-breakpoint/A1V2-DEPTH-REBUILD.md), true floor 0/447 |
| [E9](experiments/E9-compaction-cycles/) | Iterated summarize-and-continue compaction accumulates revision contamination that a no-compaction instruction avoids | Exploratory, derived post-E8 Phase 0; RE-SCOPED 2026-07-19 — 0/1800 real under repeated near-lossless summarization, but the registered 30–50% compression band was never achieved (median ratio 0.94); compaction at real ratios is untested and open |
...
| **E5** | A principled revision operation with quantified effect; context management stops being folklore | REOPENED, not settled — the registered "instruction suffices" reading was itself an instrument artifact; the operator question survives only on harder, non-templated corpora |
...
The program's most important output to date is arguably not a result but a methodological one: sentence-level NLI entailment, used across this program to score whether a model still asserts a superseded claim, fires on the requirement scaffold shared between a stale sentence and its correction — not on any real assertion — and produces hypothesis-shaped false positives on templated correction corpora. It appeared five independent times in this program (E8's A2, A1, and A3 axes, E5's Arm-C, and E9's compaction arm), each shaped exactly like a real effect, including a clean rising dose-response curve. All five collapsed to zero real contamination under a claim-grounded replacement instrument. Full account: [FINDING-NLI-ARTIFACT.md](experiments/E8-instruction-breakpoint/coldstart-package/mission-x/FINDING-NLI-ARTIFACT.md); the specific consequence for E5 is the correction note linked above. Human κ-validation of the replacement instrument's screens is staged and pending — the one load-bearing step not yet complete.
