...
- Interference grows as overwritten information accumulates (arXiv:2506.08184, body-confirmed); no cited work tests **scoped-exception generalization** — a case-limited correction treated as revoking the whole rule. `must_persist` carries the rule's unexcepted remainder, so the instrument measures the failure directly; registered as an original prediction.
...
2. **Freeze.** The frozen axes, dose levels, break threshold, per-axis probe budget, and any instrument amendment are committed, tagged, and Zenodo-archived before any probe generation ([0006 as amended 2026-07-16](../../decisions/0006-reproducibility-and-freeze.md): the public freeze commit plus the Zenodo-archived release constitute the registration act; the DOI's external timestamp is what counts).
