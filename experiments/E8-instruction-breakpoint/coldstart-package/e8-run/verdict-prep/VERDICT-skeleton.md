...
- Families entering each axis's registered run (post A-dependency-filter): A1 **{{a1_n_families}}**
  · A2 **{{a2_n_families}}** · A3 **{{a3_n_families}}**.
- A-dependency filter exclusions (pre-registered, PHASE0.md §4), counted per axis: A1
  **{{a1_n_excluded}}** / {{a1_n_families_pre_filter}} · A2 **{{a2_n_excluded}}** /
  {{a2_n_families_pre_filter}} · A3 **{{a3_n_excluded}}** / {{a3_n_families_pre_filter}}.
- Pruning register (≥2-of-3 correction-state rule, PHASE0.md §4), items dropped per axis: A1
  **{{a1_n_pruned}}** · A2 **{{a2_n_pruned}}** · A3 **{{a3_n_pruned}}**.
...
| D2 | {{a1_d2_count}} | {{a1_d2_trials}} | {{a1_d2_rate}} |
| D3 | {{a1_d3_count}} | {{a1_d3_trials}} | {{a1_d3_rate}} |

- Cochran-Armitage trend Z = **{{a1_ca_z}}**, one-sided p = **{{a1_ca_p}}** →
  `ca_uptrend = {{a1_ca_uptrend}}`.
- Strict observed rise (p̂(D1) < p̂(D2) < p̂(D3)): **{{a1_strict_rise}}**.
- `monotonicity_gate` pass (both conjuncts): **{{a1_gate_pass}}**.
- Top-level crossing: exact one-sided binomial test of D3's pooled count against θ=5% at
  α_corrected = 0.05/3 = 0.016667 → critical count k\* = **{{a1_critical_k}}**, observed count
  {{a1_d3_count}}, exact upper-tail p = **{{a1_crossing_p}}**, `crossed = {{a1_crossed}}`.
...
| Level | count | trials (N) | rate |
|---|---|---|---|
| S1 | {{a2_s1_count}} | {{a2_s1_trials}} | {{a2_s1_rate}} |
| S2 | {{a2_s2_count}} | {{a2_s2_trials}} | {{a2_s2_rate}} |
| S3 | {{a2_s3_count}} | {{a2_s3_trials}} | {{a2_s3_rate}} |

- Cochran-Armitage trend Z = **{{a2_ca_z}}**, one-sided p = **{{a2_ca_p}}** →
  `ca_uptrend = {{a2_ca_uptrend}}`.
- Strict observed rise (p̂(S1) < p̂(S2) < p̂(S3)): **{{a2_strict_rise}}**.
- `monotonicity_gate` pass: **{{a2_gate_pass}}**.
- Top-level crossing: exact one-sided binomial test of S3's pooled count against θ_persist=10%
  at α_corrected = 0.016667 → critical count k\* = **{{a2_critical_k}}**, observed count
  {{a2_s3_count}}, exact upper-tail p = **{{a2_crossing_p}}**, `crossed = {{a2_crossed}}`.
...
| Level | count | trials (N) | rate |
|---|---|---|---|
| C1 | {{a3_c1_count}} | {{a3_c1_trials}} | {{a3_c1_rate}} |
| C2 | {{a3_c2_count}} | {{a3_c2_trials}} | {{a3_c2_rate}} |
| C3 | {{a3_c3_count}} | {{a3_c3_trials}} | {{a3_c3_rate}} |

- Cochran-Armitage trend Z = **{{a3_ca_z}}**, one-sided p = **{{a3_ca_p}}** →
  `ca_uptrend = {{a3_ca_uptrend}}`.
- Strict observed rise (p̂(C1) < p̂(C2) < p̂(C3)): **{{a3_strict_rise}}**.
- `monotonicity_gate` pass: **{{a3_gate_pass}}**.
- Top-level crossing: exact one-sided binomial test of C3's pooled count against θ=5% at
  α_corrected = 0.016667 → critical count k\* = **{{a3_critical_k}}**, observed count
  {{a3_c3_count}}, exact upper-tail p = **{{a3_crossing_p}}**, `crossed = {{a3_crossed}}`.
- **A3 BREAK: {{a3_break_verdict}}** (all three conjuncts required).
...
- Oracle PASS confirmed per axis (per-task ==, fresh 1-thread re-score, stratified across every
  distinct worker signature — `oracle_verify.py`): A1 **{{a1_oracle}}** ({{a1_oracle_n}}
  tasks sampled, {{a1_oracle_mismatches}} mismatches) · A2 **{{a2_oracle}}**
  ({{a2_oracle_n}}, {{a2_oracle_mismatches}}) · A3 **{{a3_oracle}}** ({{a3_oracle_n}},
  {{a3_oracle_mismatches}}).
