...
def score_draws(top: dict[str, dict], gen_log: Path, threshold: float,
    ...
    out: dict[str, dict] = {}
    total = len(top)
    for fi, (fam, task) in enumerate(top.items(), 1):
        mc = task["must_change"]
        ...
        print(f"[{_dt.datetime.now(_dt.timezone.utc).strftime(chr(37)+chr(89)+chr(45)+chr(37)+chr(109)+chr(45)+chr(37)+chr(100)+chr(84)+chr(37)+chr(72)+chr(58)+chr(37)+chr(77)+chr(58)+chr(37)+chr(83))}Z] [filter-score] {fi}/{total} families scored", flush=True)
    return out
